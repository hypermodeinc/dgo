...
func (c *dgraphClient) StreamSnapshot(ctx context.Context, opts ...grpc.CallOption) (Dgraph_StreamSnapshotClient, error) {
	stream, err := c.cc.NewStream(ctx, &Dgraph_ServiceDesc.Streams[0], Dgraph_StreamSnapshot_FullMethodName, opts...)
	if err != nil {
		return nil, err
	}
	x := &dgraphStreamSnapshotClient{stream}
	return x, nil
}
...
func (x *dgraphStreamSnapshotClient) CloseAndRecv() (*ReceiveSnapshotKVRequest, error) {
...
func _Dgraph_StreamSnapshot_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(DgraphServer).StreamSnapshot(&dgraphStreamSnapshotServer{stream})
}
...
func (x *dgraphStreamSnapshotServer) SendAndClose(m *ReceiveSnapshotKVRequest) error {
